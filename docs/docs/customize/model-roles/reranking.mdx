...
Ollama now supports reranking using any chat model. This uses the same prompt-based approach as the LLM reranker, but works with local Ollama models.
...
:::note
Ollama reranking uses the same prompt-based approach as the LLM reranker, which may be slower and more expensive than specialized reranking models. For better performance, consider using dedicated reranking models like Voyage AI's rerank-2.
:::
